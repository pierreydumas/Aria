...
async def generate_embedding(text: str) -> list[float]:
    ...
    The local fallback produces a deterministic hash-based vector
    that is L2-normalized. Fallback vectors occupy a different region
    of the vector space from learned embeddings; cosine similarity
    ...
    def _local_embedding_fallback(value: str, dims: int = 768) -> list[float]:
        ...
        vector = [0.0] * dims
        if not tokens:
            # Return a zero vector so it won't spuriously match anything
            return vector
...
async def store_semantic_memory(
    ...
    # Tag metadata if embedding was generated by local fallback
    is_fallback = bool(_EMBED_REMOTE_DISABLED_UNTIL and datetime.now(timezone.utc) < _EMBED_REMOTE_DISABLED_UNTIL)
    ...
    memory = SemanticMemory(
        ...
        source=source,
        metadata_json={"original_length": len(content), "is_fallback_embedding": is_fallback, **(metadata or {})},
    )
    db.add(memory)
    await db.commit()
    await db.refresh(memory)
