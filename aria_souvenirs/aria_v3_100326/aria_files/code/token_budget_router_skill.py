...
Dynamically routes requests to cheaper models based on task complexity classification.
...
class TaskComplexityAnalyzer:
    ...
    CREATIVE_KEYWORDS = [
        ...
        "imagine", "invent", "brainstorm", "poetry"
    ]
    
    ANALYSIS_KEYWORDS = [
        "analyze", "compare", "evaluate", "assess", "review",
        "synthesize", "critique", "examine", "investigate"
    ]
    
    REASONING_KEYWORDS = [
        "explain", "why", "reason", "logic", "deduce",
        "infer", "prove", "demonstrate", "justify"
    ]
    ...
    def analyze(self, prompt: str, context: Optional[Dict] = None) -> TaskMetrics:
        ...
        # Estimate output tokens based on prompt length and category
        ...
        requires_accuracy = category in [TaskCategory.CODE, TaskCategory.ANALYSIS]
    ...
    def _calculate_complexity(self, prompt: str, category: TaskCategory) -> float:
        ...
        length_factor = min(len(prompt) / 1000, 0.2)  # Up to +0.2 for long prompts
        
        # Check for multi-part requests
        parts = len(re.findall(r'\d+\.|\n\n|additionally|also|furthermore', prompt.lower()))
        multi_part_factor = min(parts * 0.1, 0.2)
        
        # Check for specific requirements
        strict_requirements = len(re.findall(r'required|must|should|exactly|precisely', prompt.lower()))
        requirements_factor = min(strict_requirements * 0.05, 0.1)
        
        complexity = base_complexity + length_factor + multi_part_factor + requirements_factor
        return min(complexity, 1.0)
    
    def _estimate_output_tokens(self, prompt: str, category: TaskCategory) -> int:
        """Estimate output token count based on category and prompt."""
        base_multipliers = {
            TaskCategory.CODE: 1.5,
            TaskCategory.CREATIVE: 2.0,
            TaskCategory.ANALYSIS: 1.8,
            TaskCategory.CONVERSATIONAL: 0.8,
            TaskCategory.ROUTINE: 0.5,
            TaskCategory.COMPLEX: 2.5
        }
        
        # Base estimate on input length
        base_estimate = len(prompt) * base_multipliers[category]
        
        # Cap reasonable ranges
        min_tokens = 50
        max_tokens = 4000
        
        return int(max(min_tokens, min(base_estimate, max_tokens)))
...
class TokenBudgetRouter:
    ...
    def route(self, 
              ...
              force_tier: Optional[ModelTier] = None) -> Dict:
        ...
        if force_tier:
            ...
            selected_tier = self._select_tier(metrics)
        
        # Check budget constraints
        budget_status = self._check_budget()
        if budget_status == "critical":
            # Force downgrade to local/free tier
    ...
    def _downgrade_tier(self, tier: ModelTier) -> ModelTier:
        ...
        downgrade_map = {
            ModelTier.EXPERT: ModelTier.PREMIUM,
            ModelTier.PREMIUM: ModelTier.FREE,
            ModelTier.FREE: ModelTier.LOCAL,
            ModelTier.LOCAL: ModelTier.LOCAL
        }
        return downgrade_map.get(tier, ModelTier.LOCAL)
    
    def _select_model(self, tier: ModelTier) -> str:
        """Select specific model from tier."""
        models = self.MODEL_ROUTES.get(tier, ["qwen3-mlx"])
        # In production, could use load balancing or health checks
        return models[0]
    ...
    def _explain_routing(self, metrics: TaskMetrics, tier: ModelTier, budget: str) -> str:
        ...
        reasons.append(f"Task category: {metrics.category.value}")
        reasons.append(f"Complexity score: {metrics.complexity_score:.2f}")
        
        if metrics.requires_accuracy:
            reasons.append("Requires high accuracy")
        if metrics.requires_reasoning:
            reasons.append("Requires reasoning")
        if metrics.requires_creativity:
            reasons.append("Creative task")
        
        if budget == "critical":
            reasons.append("Budget CRITICAL - forced to local tier")
        elif budget == "warning":
            reasons.append("Budget WARNING - downgraded to save tokens")
    ...
    def get_usage_stats(self) -> Dict:
        ...
        return {
            ...
            "total_routes": len(self._usage_log),
...
# Skill interface for Aria Engine
def analyze_task(prompt: str, context: Optional[Dict] = None) -> Dict:
    """Analyze task complexity."""
    analyzer = TaskComplexityAnalyzer()
    metrics = analyzer.analyze(prompt, context)
    return {
        "category": metrics.category.value,
        "complexity_score": metrics.complexity_score,
        "estimated_output_tokens": metrics.estimated_output_tokens,
        "requires_reasoning": metrics.requires_reasoning,
        "requires_creativity": metrics.requires_creativity,
        "requires_accuracy": metrics.requires_accuracy
    }

def route_request(prompt: str, 
                  context: Optional[Dict] = None,
                  daily_budget: int = 100000) -> Dict:
    """Route a request to appropriate model."""
    router = TokenBudgetRouter(daily_budget=daily_budget)
...
if __name__ == "__main__":
    ...
    test_prompts = [
        ...
        "Create a short story about a robot learning to feel",
        "Debug this SQL query that's returning null",
    ]
    ...
    for prompt in test_prompts:
        ...
        print(f"  → Budget: {result['budget']['remaining']} remaining")
